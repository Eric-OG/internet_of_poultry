...
#define nameMapType std::map<std::uint32_t, String>
...
class namedMesh : public painlessMesh {
 public:
  ...
  JsonObject addressToNameJson() {
    ...
    JsonObject nameMapJson = doc.to<JsonObject>();
    char keyStr[11];

    for (const auto &item : nameMap) {
      utoa(item.first, keyStr, 10);
      nameMapJson[keyStr] = item.second;
    }
    utoa(getNodeId(), keyStr, 10);
    nameMapJson[keyStr] = getName();
