...
class CUDP {
 ...
 public:
  String packageData(JsonObject app_data) {
    String app_data_str;
    char checksum_hex[9];
  ...
  bool unpackData(const String &cudp_packet, JsonObject *app_data) {
    ...
    String app_data_str = cudp_packet.substring(0, string_length - 10);
    String checksum_str = cudp_packet.substring(string_length - 10);
    crc.add((uint8_t *)app_data_str.c_str(), strlen(app_data_str.c_str()));
    ...
    StaticJsonDocument<256> app_data_doc;
